...
void write_hard_configs(const char * filename)
{
    ...
    };
    {
        json camera_json{
            ...
            {"ns", 50},
            {"depth", 10},
            {"cam_filename", (string("pictures/dir_9/make_2_conf_3_")+to_string(1)+string(".ppm"))}
        };
    ...
    for (int i=1; i<20; ++i)
    {
        json camera_json{
            ...
            {"ns", 20},
            {"depth", 10},
            {"cam_filename", (string("pictures/dir_9/make_2_conf_3_")+to_string(i+1)+string(".ppm"))}
        };
        world_json["cameras"].push_back(camera_json);
    }
...
int main(){
    write_hard_configs("make_2_config_9_made.json");
}